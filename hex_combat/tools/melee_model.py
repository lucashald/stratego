...
def _comparative_pool(
    unit: Unit, opponent: Unit, role_die: bool, rng: random.Random
) -> tuple[int, int]:
    """The pool the shipped rules actually roll: 1 base d6, +1 if strictly
    heavier than the opponent, +1 if strictly stronger, +1 for the role die.
    Returns (highest die, count of 6s)."""
    count = (
        1
        + (1 if WEIGHT_RANK[unit.weight] > WEIGHT_RANK[opponent.weight] else 0)
        + (1 if unit.strength > opponent.strength else 0)
        + (1 if role_die else 0)
    )
    dice = [rng.randint(1, 6) for _ in range(count)]
    return max(dice), sum(1 for d in dice if d == 6)


def rule_baseline_v1(attacker: Unit, defender: Unit, rng: random.Random) -> ClashResult:
    """THE SHIPPED RULE - mirrors _resolve_battle in scripts/stratego_game.gd.

    Comparative dice as in comparative_dice_marginmax, but the two things that
    variant still carried are gone: no armor anywhere, and damage is the pure
    score margin rather than max(margin, die). Score is highest-die + strength.

    Damage has two independent sources. The margin is paid only by the loser.
    Surviving 6s are paid by whoever failed to match them, win, lose or draw -
    so a formation being overrun can still put one through the winner, and a
    draw is free of margin damage but not of crits. Because 6s cancel one for
    one, at most one side is ever owed crit damage.

    Deliberately the stripped-down baseline. Cover and high ground get measured
    against this, one at a time."""
    a_roll, a_sixes = _comparative_pool(attacker, defender, attacker.role == "cavalry", rng)
    d_roll, d_sixes = _comparative_pool(defender, attacker, defender.role == "infantry", rng)
    a_score = a_roll + attacker.strength
    d_score = d_roll + defender.strength

    a_damage = max(0, d_score - a_score) + max(0, d_sixes - a_sixes)
    d_damage = max(0, a_score - d_score) + max(0, a_sixes - d_sixes)
    if a_score > d_score:
        return ClashResult("attacker", a_damage, d_damage)
    if d_score > a_score:
        return ClashResult("defender", a_damage, d_damage)
    return ClashResult("tie", a_damage, d_damage)


RULES = {
    "current": rule_current,
    "baseline_v1": rule_baseline_v1,
...
def _ranged_baseline(archer: Unit, target: Unit, rng: random.Random, short_shot: bool) -> RangedResult:
    """THE SHIPPED RANGED RULE - mirrors calculate_ranged in
    scripts/stratego_game.gd. The same shape as rule_baseline_v1 minus the
    role die: the archer's extra die is the short shot, and the target rolls a
    comparative pool of its own with no role or range die. A hit needs the
    archer's score to be strictly higher, and damage is the margin plus
    surviving 6s. As in archer_v4, an uncancelled 6 chips through even on a
    lost contest, so an outmatched archer is never firing for nothing; the
    target's own 6s are its only answer to that. `hit` means the archer won the
    contest, which is not the same as having done damage - a graze is a miss
    that still drew blood.

    Registered twice, as short and long, because the two ranges differ only in
    that one die and the whole point is to be able to price it."""
    a_roll, a_sixes = _comparative_pool(archer, target, short_shot, rng)
    d_roll, d_sixes = _comparative_pool(target, archer, False, rng)
    a_score = a_roll + archer.strength
    d_score = d_roll + target.strength
    damage = max(0, a_score - d_score) + max(0, a_sixes - d_sixes)
    return RangedResult(a_score > d_score, damage)


def ranged_rule_baseline_v1_short(archer: Unit, target: Unit, rng: random.Random) -> RangedResult:
    return _ranged_baseline(archer, target, rng, True)


def ranged_rule_baseline_v1_long(archer: Unit, target: Unit, rng: random.Random) -> RangedResult:
    return _ranged_baseline(archer, target, rng, False)


RANGED_RULES = {
    "archer_v1": ranged_rule_v1,
    "archer_baseline_v1_short": ranged_rule_baseline_v1_short,
    "archer_baseline_v1_long": ranged_rule_baseline_v1_long,
